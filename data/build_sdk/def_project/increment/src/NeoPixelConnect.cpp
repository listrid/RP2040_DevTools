...
_NeoPixelConnect::_NeoPixelConnect(uint8_t pinNumber, uint16_t numberOfPixels) {
    this->pixelSm = 0;
    this->pixelPio = pio0;
...
_NeoPixelConnect::_NeoPixelConnect(uint8_t pinNumber, uint16_t numberOfPixels, PIO pio, uint sm) {
    this->pixelSm = sm;
    this->pixelPio = pio;
    this->neoPixelInit(pinNumber, numberOfPixels);
}
...
/// @param pinNumber: GPIO pin that controls the NeoPixel string.
/// @param numberOfPixels: Number of pixels in the string
void _NeoPixelConnect::neoPixelInit(uint8_t pinNumber, uint16_t numberOfPixels) {
...
void _NeoPixelConnect::neoPixelSetValue(uint16_t pixelNumber, uint8_t r, uint8_t g, uint8_t b, bool autoShow) {
...
/// @param autoShow: If true, show the change immediately
void _NeoPixelConnect::neoPixelClear(bool autoShow) {
...
/// @param b: blue value (0-255)
/// @param autoShow: If true, show the change immediately.
void _NeoPixelConnect::neoPixelFill(uint8_t r, uint8_t g, uint8_t b, bool autoShow) {
...
void _NeoPixelConnect::neoPixelShow(void) {
...
/// @param g: green value(0-255)
/// @param b: blue value (0-255)
uint32_t _NeoPixelConnect::urgb_u32(uint8_t r, uint8_t g, uint8_t b) {
    return
            ((uint32_t)(r) << 8) |
            ((uint32_t)(g) << 16) |
            (uint32_t)(b);
...
void _NeoPixelConnect::putPixel(uint32_t pixel_grb) {
