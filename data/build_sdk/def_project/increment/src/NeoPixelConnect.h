...
#ifndef NEOPIXEL_CONNECT_NEOPIXELCONNECT_H_
#define NEOPIXEL_CONNECT_NEOPIXELCONNECT_H_
...
public:
    ...
    _NeoPixelConnect(uint8_t pinNumber, uint16_t numberOfPixels);

    /// @brief Constructor
    /// @param pinNumber: GPIO pin that controls the NeoPixel string.
    /// @param numberOfPixels: Number of pixels in the string
    /// This constructor sets pio=pio0 and sm to 0
    _NeoPixelConnect(uint8_t pinNumber, uint16_t numberOfPixels, PIO pio, uint sm);

    /// @brief Destructor
    virtual ~_NeoPixelConnect(){};
